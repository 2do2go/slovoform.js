...
(function () {
	...
	Pluralize.prototype.get = function(str, number) {
		var self = this;
		// replace each word in the string to pluralizing form
		return str.replace(/[а-яА-ЯёЁa-zA-Z]+/ig, function(word) {
			var article = self._dict[word.toLowerCase()],
				result = word;
			if (article) {
				result = self._strategy(article, number);
				var isCapitalized = word.toUpperCase()[0] == word[0];
				if (self._options.preserveCaseOnGet && isCapitalized) {
